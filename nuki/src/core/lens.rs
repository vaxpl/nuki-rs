...
//! ```
//! use nuki::{Data, Lens, Widget, WidgetExt, widget::{TextBox, Flex}};
//!
...
pub trait LensExt<A: ?Sized, B: ?Sized>: Lens<A, B> {
    ...
    ///
    /// ```
    /// use nuki::{widget::TextBox, Data, Lens, LensExt, Widget, WidgetExt};
    ///
...
///
/// ```
/// let lens = nuki::lens::Field::new(|x: &Vec<u32>| &x[42], |x| &mut x[42]);
/// ```
...
/// ```
/// struct Foo { x: u32 }
/// let lens = nuki::lens!(Foo, x);
/// let lens = nuki::lens!((u32, bool), 1);
/// let lens = nuki::lens!([u8], [4]);
/// ```
#[macro_export]
macro_rules! lens {
    ($ty:ty, [$index:expr]) => {
