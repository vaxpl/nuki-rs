...
/// can be expensive. To use collection types with nuki, there are two easy options:
/// either wrap the collection in an `Arc`, or build nuki with the `im` feature,
/// which adds `Data` implementations to the collections from the [`im` crate],
/// a set of immutable data structures that fit nicely with nuki.
///
