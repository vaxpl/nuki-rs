...
///
/// ```
///# use nuki::{Key, Color, AppLauncher, widget::Label};
/// const IMPORTANT_LABEL_COLOR: Key<Color> = Key::new("org.nuki.example.important-label-color");
///
...
///             env.set(IMPORTANT_LABEL_COLOR, Color::rgb_f(1.0, 0.0, 0.0));
///         });
/// }
/// ```
///
...
impl<T> Key<T> {
    ...
    /// use nuki::core::Key;
    ///
    /// let float_key: Key<f64> = Key::new("org.nuki.example.a.very.good.float");
    /// ```
...
mod tests {
    ...
    fn string_key_or_value() {
        const MY_KEY: Key<ArcStr> = Key::new("org.nuki.test.my-string-key");
