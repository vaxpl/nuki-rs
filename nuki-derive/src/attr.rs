...
const BASE_NUKI_DEPRECATED_ATTR_PATH: &str = "nuki";
...
impl Field<DataAttrs> {
    pub fn parse_ast(field: &syn::Field, index: usize) -> Result<Self, Error> {
        ...
        for attr in field.attrs.iter() {
            if attr.path.is_ident(BASE_NUKI_DEPRECATED_ATTR_PATH) {
                panic!(
                    "The 'nuki' attribute has been replaced with separate \
    ...
    pub fn same_fn_path_tokens(&self) -> TokenStream {
        match self.attrs.same_fn {
            ...
            None => {
                ...
                quote_spanned!(span=> nuki::Data::same)
            }
        }
    }
}
